...
#define TCPCB_SACKED_ACKED	0x01	/* SKB ACK'd by a SACK block被确认了 ----被 SACK 块 ACK'd 也就是SACK块已经给出了skb数据缓冲区中的段回答信息 	*/

//标识skb是否被重传过，标识为字母R
#define TCPCB_SACKED_RETRANS	0x02	/* SKB retransmitted数据段被重传		 用于处理基于SACK 的重传情况  表示 此数据包是因为sack重导致传不是rto*/
...
#define TCPCB_EVER_RETRANS	0x80	/* Ever retransmitted frame 调用transmit_skb后设置此参数；指明数据段以前重传过 	
				一旦数据包因任何原因（例如超时或丢包）被重传，这个标志就会被设置*/
 ...
 * ssthresh.
 函数用于计算 TCP 的当前慢启动阈值（ssthresh），即决定当拥塞窗口（cwnd）达到多大时，TCP 进入拥塞避免阶段。
 这个函数会根据 cwnd 和当前拥塞状态动态调整 ssthresh，为优化传输速率和减少网络拥塞提供基础。
 */
static inline __u32 tcp_current_ssthresh(const struct sock *sk)
{
	...
	if (tcp_in_cwnd_reduction(sk))
		return tp->snd_ssthresh;//则 ssthresh 保持不变，返回当前的 tp->snd_ssthresh 
	else
		return max(tp->snd_ssthresh,
			   ((tp->snd_cwnd >> 1) +
			    (tp->snd_cwnd >> 2)));//ssthresh 会被提高至 cwnd 的 3/4 和 ssthresh 最大值中一个
}
...
static inline u32 tcp_highest_sack_seq(struct tcp_sock *tp)
{
	...
	if (tp->highest_sack == NULL)//sacked_out>0,会出现highest_sack为NULL的场景???	
		return tp->snd_nxt;
...
//最大的己被SACK确认的skb的指针指向队首,这种设置也很奇怪,因为对首skb毕竟还没有被SACK
